...
/**
 * Loads the Phase 4 complex securities position reconciliation example.
 */
@Component
@Order(20)
public class SecuritiesPositionEtlPipeline extends AbstractSampleEtlPipeline {
    ...
    public void run() {
        ...
        List<SourceRecordA> sourceARecords = readCsv("etl/complex/securities_source_a.csv").stream()
                .map(row -> mapSourceA(definition, row))
                .toList();
        sourceRecordARepository.saveAll(sourceARecords);

        List<SourceRecordB> sourceBRecords = readCsv("etl/complex/securities_source_b.csv").stream()
